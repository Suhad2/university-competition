@extends('layouts.app')

@section('title', 'Exam Manager Dashboard - University Competition')

@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="fas fa-play-circle"></i> Exam Manager Dashboard</h2>
        <div class="text-muted">
            <small>Welcome, {{ Auth::user()->name }}</small>
        </div>
    </div>

    <!-- Test Status Card -->
    <div class="row mb-4">
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h5><i class="fas fa-info-circle"></i> Current Test Status</h5>
                </div>
                <div class="card-body">
                    @if ($currentTest)
                        <div class="row">
                            <div class="col-md-3">
                                <div class="stat-card">
                                    <h6>Status</h6>
                                    @if ($currentTest->isWaiting())
                                        <span class="badge bg-warning fs-6">Waiting</span>
                                    @elseif($currentTest->isActive())
                                        <span class="badge bg-success fs-6">Active</span>
                                    @elseif($currentTest->isEnded())
                                        <span class="badge bg-secondary fs-6">Ended</span>
                                    @endif
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="stat-card">
                                    <h6>Total Questions</h6>
                                    <h4>{{ $stats['total_questions'] }}</h4>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="stat-card">
                                    <h6>Total Users</h6>
                                    <h4>{{ $stats['waiting_users'] }}</h4>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="stat-card">
                                    <h6>Ready Participants</h6>
                                    <h4 id="ready-count">{{ $stats['ready_participants'] }}</h4>
                                </div>
                            </div>


                        </div>
                    @else
                        <div class="text-center">
                            <i class="fas fa-info-circle fa-3x text-muted mb-3"></i>
                            <h5>No Active Test</h5>
                            <p class="text-muted">Start a new test to begin the competition</p>
                        </div>
                    @endif
                </div>
            </div>
        </div>
    </div>

    <!-- Control Buttons -->
    <div class="row mb-4">
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h5><i class="fas fa-cogs"></i> Test Controls</h5>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-3">
                            @if (!$currentTest || $currentTest->isEnded())
                                <form method="POST" action="{{ route('exam-manager.start-test') }}">
                                    @csrf
                                    <button type="submit" class="btn btn-success btn-lg w-100">
                                        <i class="fas fa-play"></i> Start Test
                                    </button>
                                </form>
                            @else
                                <button class="btn btn-success btn-lg w-100" disabled>
                                    <i class="fas fa-play"></i> Test Running
                                </button>
                            @endif
                        </div>
                        <div class="col-md-3">
                            @if ($currentTest && $currentTest->isWaiting() && !$currentTest->currentQuestion)
                                <form method="POST" action="{{ route('exam-manager.start-first-question') }}">
                                    @csrf
                                    <button type="submit" class="btn btn-info btn-lg w-100"
                                        onclick="return confirm('Start the first question for {{ $stats['ready_participants'] }} ready participants?')">
                                        <i class="fas fa-rocket"></i> Start First Question
                                    </button>
                                </form>
                            @elseif($currentTest && $currentTest->isActive())
                                <form method="POST" action="{{ route('exam-manager.next-question') }}">
                                    @csrf
                                    <button type="submit" class="btn btn-primary btn-lg w-100">
                                        <i class="fas fa-forward"></i> Next Question
                                    </button>
                                </form>
                            @else
                                <button class="btn btn-primary btn-lg w-100" disabled>
                                    <i class="fas fa-forward"></i> Next Question
                                </button>
                            @endif
                        </div>
                        <div class="col-md-3">
                            @if ($currentTest && ($currentTest->isWaiting() || $currentTest->isActive()))
                                <form method="POST" action="{{ route('exam-manager.end-test') }}"
                                    onsubmit="return confirm('Are you sure you want to end the test? This action cannot be undone.')">
                                    @csrf
                                    <button type="submit" class="btn btn-danger btn-lg w-100">
                                        <i class="fas fa-stop"></i> End Test
                                    </button>
                                </form>
                            @else
                                <button class="btn btn-danger btn-lg w-100" disabled>
                                    <i class="fas fa-stop"></i> End Test
                                </button>
                            @endif
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Current Question Display -->
    @if ($currentTest && $currentTest->isActive() && $currentTest->currentQuestion)
        <div class="row mb-4">
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h5><i class="fas fa-question-circle"></i> Current Question</h5>
                    </div>
                    <div class="card-body">
                        <div class="question-display">
                            <h4>{{ $currentTest->currentQuestion->title }}</h4>
                            <div class="row mt-3">
                                <div class="col-md-6">
                                    <div class="option-display">
                                        <strong>A.</strong> {{ $currentTest->currentQuestion->option_a }}
                                    </div>
                                    <div class="option-display">
                                        <strong>B.</strong> {{ $currentTest->currentQuestion->option_b }}
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="option-display">
                                        <strong>C.</strong> {{ $currentTest->currentQuestion->option_c }}
                                    </div>
                                    <div class="option-display">
                                        <strong>D.</strong> {{ $currentTest->currentQuestion->option_d }}
                                    </div>
                                </div>
                            </div>
                            <div class="mt-3">
                                <span class="badge bg-info">Correct Answer:
                                    {{ $currentTest->currentQuestion->correct_answer }}</span>
                                @if ($currentTest->question_start_time)
                                    <span class="badge bg-warning ms-2">
                                        Time Remaining: {{ $currentTest->getTimeRemaining() }}s
                                    </span>
                                @endif
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    @endif

    <!-- Participants Status -->
    @if ($currentTest && ($currentTest->isActive() || $currentTest->isWaiting()))
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h5><i class="fas fa-users"></i>
                            @if ($currentTest->isWaiting())
                                Ready Participants ({{ $stats['ready_participants'] }})
                            @else
                                Participants Status
                            @endif
                        </h5>
                    </div>
                    <div class="card-body">
                        @if ($stats['ready_participants'] == 0)
                            <div class="alert alert-warning">
                                <i class="fas fa-exclamation-triangle"></i>
                                No participants are ready yet. Wait for students to click "I'm Ready".
                            </div>
                        @else
                            <div class="table-responsive">
                                <table class="table table-striped">
                                    <thead>
                                        <tr>
                                            <th>Name</th>
                                            <th>University</th>
                                            <th>Status</th>
                                            <th>Answer</th>
                                        </tr>
                                    </thead>
                                    <tbody id="participantsTable">
                                        @php
                                            $readyParticipants = $currentTest->getReadyParticipants();
                                        @endphp
                                        @foreach ($users as $user)
                                            @if (in_array($user->id, $readyParticipants))
                                                <tr>
                                                    <td>{{ $user->name }}</td>
                                                    <td>{{ $user->university ?? 'N/A' }}</td>
                                                    <td>
                                                        @php
                                                            $hasAnswered = \App\Models\Answer::where(
                                                                'test_id',
                                                                $currentTest->id,
                                                            )
                                                                ->where('user_id', $user->id)
                                                                ->where(
                                                                    'question_id',
                                                                    $currentTest->current_question_id ?? 0,
                                                                )
                                                                ->exists();
                                                        @endphp
                                                        @if ($hasAnswered)
                                                            <span class="badge bg-success">Answered</span>
                                                        @else
                                                            @if ($currentTest->isWaiting())
                                                                <span class="badge bg-info">Ready</span>
                                                            @else
                                                                <span class="badge bg-warning">Waiting</span>
                                                            @endif
                                                        @endif
                                                    </td>
                                                    <td>
                                                        @if ($hasAnswered)
                                                            @php
                                                                $answer = \App\Models\Answer::where(
                                                                    'test_id',
                                                                    $currentTest->id,
                                                                )
                                                                    ->where('user_id', $user->id)
                                                                    ->where(
                                                                        'question_id',
                                                                        $currentTest->current_question_id ?? 0,
                                                                    )
                                                                    ->first();
                                                            @endphp
                                                            {{ $answer->selected_answer ?? 'N/A' }}
                                                        @else
                                                            <span class="text-muted">-</span>
                                                        @endif
                                                    </td>
                                                </tr>
                                            @endif
                                        @endforeach
                                    </tbody>
                                </table>
                            </div>
                        @endif
                    </div>
                </div>
            </div>
        </div>
    @endif
    <style>
        .stat-card {
            text-align: center;
            padding: 1rem;
        }

        .stat-card h6 {
            margin-bottom: 0.5rem;
            color: #6c757d;
            font-size: 0.9rem;
        }

        .stat-card h4 {
            margin-bottom: 0;
            color: #007bff;
        }

        .option-display {
            padding: 0.5rem;
            margin: 0.25rem 0;
            background: #f8f9fa;
            border-radius: 0.25rem;
        }

        .question-display {
            max-height: 300px;
            overflow-y: auto;
        }
    </style>

    <!-- Auto-refresh script -->
    <script>
        document.addEventListener('DOMContentLoaded', function() {

            console.log('Exam Manager Dashboard loaded');

            // Application state
            const state = {
                currentTest: null,
                currentQuestion: null,
                questionStartTime: null,
                timeLimit: 35,
                timeRemaining: 0,
                timerInterval: null,
                participants: []
            };

            /* ===============================
               Prevent automatic form submit
            =============================== */
            const forms = document.querySelectorAll('form');
            forms.forEach(form => {
                form.addEventListener('submit', function(e) {
                    const submitter = e.submitter;
                    if (submitter && submitter.tagName === 'BUTTON') {
                        return true; // allow normal submit
                    }
                });
            });

            /* ===============================
               Initialize Echo/Pusher
            =============================== */
            if (typeof Echo === 'undefined') {
                console.error('Echo is not defined, polling will be used as fallback');
            } else {
                const channel = Echo.channel('quiz-participants');

                channel.subscribed(function() {
                    console.log('✓ Successfully subscribed to quiz-participants channel');
                }).error(function(error) {
                    console.error('❌ Channel subscription error:', error);
                });

                // Listen for all events
                channel.listen('*', function(e) {
                    console.log('🎯 EVENT RECEIVED:', e.event, e.data);
                });

                // Test Started Event
                channel.listen('.test.started', function(e) {
                    console.log('✅ Test started:', e);
                    fetchUpdates();
                });

                // Test Updated Event
                channel.listen('.test.updated', function(e) {
                    console.log('📊 Test updated:', e);
                    if (e.stats) {
                        updateReadyCount(e.stats.ready_participants);
                    }
                    if (e.participants) {
                        state.participants = e.participants;
                        updateParticipantsTable(e.participants);
                    }
                    if (e.test) {
                        updateTestStatus(e.test);
                    }
                });

                // Participant Ready Event
                channel.listen('.participant.ready', function(e) {
                    console.log('👤 Participant ready:', e);
                    if (e.ready_count !== undefined) {
                        updateReadyCount(e.ready_count);
                    }
                    fetchUpdates();
                });

                // Question Started Event
                channel.listen('.question.started', function(e) {
                    console.log('❓ Question started:', e);
                    fetchUpdates();
                });

                // Answer Received Event
                channel.listen('.answer.received', function(e) {
                    console.log('✅ Answer received:', e);
                    if (e.user_id) {
                        updateParticipantAnswer(e.user_id, e.selected_answer);
                    }
                    fetchUpdates();
                });

                // Test Ended Event
                channel.listen('.test.ended', function(e) {
                    console.log('🏁 Test ended:', e);
                    showTestEndedNotification();
                });
            }

            /* ===============================
               Polling for updates (backup/fallback)
            =============================== */
            let consecutiveErrors = 0;
            const maxErrorsBeforePause = 3;
            let pollingPaused = false;
            let retryTimeout = null;

            async function fetchUpdates() {
                if (pollingPaused) {
                    return;
                }

                try {
                    const response = await axios.get('/exam-manager/poll');
                    consecutiveErrors = 0;

                    if (response.data) {
                        // Update stats
                        if (response.data.stats) {
                            updateReadyCount(response.data.stats.ready_participants);
                        }

                        // Update participants
                        if (response.data.participants) {
                            state.participants = response.data.participants;
                            updateParticipantsTable(response.data.participants);
                        }

                        // Update current question
                        if (response.data.current_question) {
                            updateCurrentQuestion(response.data.current_question);
                            state.currentQuestion = response.data.current_question;

                            if (response.data.test && response.data.test.question_start_time) {
                                state.questionStartTime = response.data.test.question_start_time;
                                startTimer();
                            }
                        }

                        // Update test status
                        if (response.data.test) {
                            updateTestStatus(response.data.test);
                            state.currentTest = response.data.test;
                        }
                    }

                } catch (error) {
                    consecutiveErrors++;
                    console.error('Polling error (attempt ' + consecutiveErrors + '):', error.message);

                    if (consecutiveErrors >= maxErrorsBeforePause) {
                        pollingPaused = true;
                        console.warn('Too many consecutive polling errors, pausing...');

                        retryTimeout = setTimeout(() => {
                            console.log('Attempting to resume polling...');
                            consecutiveErrors = 0;
                            pollingPaused = false;
                        }, 30000);
                    }
                }
            }

            // Start polling every 2 seconds
            setInterval(fetchUpdates, 2000);

            // Initial fetch
            fetchUpdates();

            /* ===============================
               UI Update Functions
            =============================== */

            function updateReadyCount(count) {
                const counter = document.getElementById('ready-count');
                if (counter) {
                    counter.textContent = count;
                }
            }

            function updateAnsweredCount(count) {
                const counter = document.getElementById('answered-count');
                if (counter) {
                    counter.textContent = count;
                }
            }

            function updateTestStatus(testData) {
                // Update status badge if exists
                const statusBadges = document.querySelectorAll('.badge');
                statusBadges.forEach(badge => {
                    if (badge.closest('.stat-card')) {
                        if (testData.is_waiting) {
                            badge.textContent = 'Waiting';
                            badge.className = 'badge bg-warning fs-6';
                        } else if (testData.is_active) {
                            badge.textContent = 'Active';
                            badge.className = 'badge bg-success fs-6';
                        } else if (testData.is_ended) {
                            badge.textContent = 'Ended';
                            badge.className = 'badge bg-secondary fs-6';
                        }
                    }
                });
            }

            function updateParticipantsTable(participants) {
                const table = document.getElementById('participantsTable');
                if (!table) return;

                const isActive = state.currentTest && state.currentTest.is_active;

                let html = '';
                participants.forEach(user => {
                    // Only show ready participants if test is waiting
                    if (!isActive && !user.is_ready) return;

                    let statusBadge = '';
                    let answerText = '-';

                    if (user.status === 'ready') {
                        statusBadge = '<span class="badge bg-info">Ready</span>';
                    } else if (user.status === 'answered') {
                        statusBadge = '<span class="badge bg-success">Answered</span>';
                        answerText = user.selected_answer || '-';
                    } else if (user.status === 'waiting') {
                        statusBadge = '<span class="badge bg-warning">Waiting</span>';
                    } else if (user.status === 'ended') {
                        statusBadge = '<span class="badge bg-secondary">Ended</span>';
                    }

                    html += `
                <tr>
                    <td>${escapeHtml(user.name)}</td>
                    <td>${escapeHtml(user.university || 'N/A')}</td>
                    <td>${statusBadge}</td>
                    <td>${answerText}</td>
                </tr>
            `;
                });

                table.innerHTML = html ||
                    '<tr><td colspan="4" class="text-center text-muted">No participants yet</td></tr>';
            }

            function updateParticipantAnswer(userId, selectedAnswer) {
                const table = document.getElementById('participantsTable');
                if (!table) return;

                // Find the participant row and update it
                const rows = table.querySelectorAll('tr');
                rows.forEach(row => {
                    const cells = row.querySelectorAll('td');
                    if (cells.length >= 4) {
                        // Check if this row matches the userId from state.participants
                        const participantName = cells[0].textContent;
                        const participant = state.participants.find(p => p.name === participantName);
                        if (participant && participant.id === userId) {
                            const statusCell = cells[2];
                            const answerCell = cells[3];
                            statusCell.innerHTML = '<span class="badge bg-success">Answered</span>';
                            answerCell.textContent = selectedAnswer || '-';
                        }
                    }
                });
            }

            function updateCurrentQuestion(question) {
                // Update question display if it exists
                const questionContainer = document.querySelector('.question-display');
                if (!questionContainer) return;

                const title = questionContainer.querySelector('h4');
                if (title) title.textContent = question.title;

                const optionDisplays = questionContainer.querySelectorAll('.option-display');
                if (optionDisplays.length >= 4) {
                    optionDisplays[0].innerHTML = '<strong>A.</strong> ' + question.option_a;
                    optionDisplays[1].innerHTML = '<strong>B.</strong> ' + question.option_b;
                    optionDisplays[2].innerHTML = '<strong>C.</strong> ' + question.option_c;
                    optionDisplays[3].innerHTML = '<strong>D.</strong> ' + question.option_d;
                }
            }

            function startTimer() {
                if (state.timerInterval) {
                    clearInterval(state.timerInterval);
                }

                if (!state.questionStartTime || !state.currentQuestion) {
                    return;
                }

                // Calculate remaining time
                const elapsed = Math.floor((Date.now() / 1000) - state.questionStartTime);
                state.timeRemaining = Math.max(0, state.timeLimit - elapsed);

                // Update timer display
                const timerElements = document.querySelectorAll('.badge.bg-warning');
                timerElements.forEach(el => {
                    if (el.textContent.includes('Time Remaining')) {
                        el.textContent = 'Time Remaining: ' + state.timeRemaining + 's';
                    }
                });

                // Start countdown
                state.timerInterval = setInterval(() => {
                    state.timeRemaining--;

                    timerElements.forEach(el => {
                        if (el.textContent.includes('Time Remaining')) {
                            el.textContent = 'Time Remaining: ' + state.timeRemaining + 's';

                            // Change color based on remaining time
                            if (state.timeRemaining <= 10) {
                                el.classList.remove('bg-warning');
                                el.classList.add('bg-danger');
                            } else if (state.timeRemaining <= 20) {
                                el.classList.remove('bg-warning');
                                el.classList.add('bg-warning'); // Keep warning
                            }
                        }
                    });

                    if (state.timeRemaining <= 0) {
                        clearInterval(state.timerInterval);
                        timerElements.forEach(el => {
                            if (el.textContent.includes('Time Remaining')) {
                                el.textContent = 'Time Remaining: 0s';
                                el.classList.remove('bg-warning');
                                el.classList.add('bg-danger');
                            }
                        });
                    }
                }, 1000);
            }

            function showTestEndedNotification() {
                // Update UI to show test ended status
                const statusBadges = document.querySelectorAll('.badge');
                statusBadges.forEach(badge => {
                    if (badge.textContent === 'Active') {
                        badge.textContent = 'Ended';
                        badge.classList.remove('bg-success');
                        badge.classList.add('bg-secondary');
                    }
                });

                // Disable control buttons
                const buttons = document.querySelectorAll(
                    'button.btn-success, button.btn-primary, button.btn-danger');
                buttons.forEach(btn => {
                    btn.disabled = true;
                });
            }

            /* ===============================
               Utility Functions
            =============================== */
            function escapeHtml(text) {
                if (!text) return '';
                const div = document.createElement('div');
                div.textContent = text;
                return div.innerHTML;
            }
        });
    </script>

@endsection
