...
<head>
    ...
    <style>
        * {
        ...
        .scoreboard-container.active.scoreboard-visible {
...
<body>
    <div class="guest-container">
        ...
        <main class="main-content">
            <div class="content-wrapper" id="content-wrapper">
                ...
                <div class="scoreboard-container {{ $currentTest && $currentTest->isEnded() ? 'active' : '' }}"
    ...
    <script>
        ...
        const EXAM_ENDED_KEY = 'guest_exam_ended';

        // Save exam ended state to localStorage
        function saveExamEndedState() {
            const endedState = {
                isEnded: true,
                endedAt: Date.now()
            };
            localStorage.setItem(EXAM_ENDED_KEY, JSON.stringify(endedState));
            console.log('Exam ended state saved to localStorage');
        }

        // Check if exam was ended from localStorage
        function getExamEndedState() {
            const savedState = localStorage.getItem(EXAM_ENDED_KEY);
            if (savedState) {
                try {
                    const endedState = JSON.parse(savedState);
                    const timeSinceEnded = Date.now() - (endedState.endedAt || 0);
                    const maxAge = 24 * 60 * 60 * 1000; // 24 hours
                    if (timeSinceEnded < maxAge && endedState.isEnded) {
                        return true;
                    }
                } catch (e) {
                    console.error('Error parsing exam ended state:', e);
                }
            }
            return false;
        }

        // Clear exam ended state from localStorage
        function clearExamEndedState() {
            localStorage.removeItem(EXAM_ENDED_KEY);
            console.log('Exam ended state cleared from localStorage');
        }
            ...
            if (savedState) {
                try {
                    ...
                    if (timeSinceSave < maxAge && questionState.question) {
                        ...
                        state.timeRemaining = Math.max(0, (questionState.timeRemaining || questionState.timeLimit) -
                            elapsedSinceSave);
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Check if exam was already ended (from localStorage or initial Blade data)
            if (getExamEndedState() || {{ $currentTest && $currentTest->isEnded() ? 'true' : 'false' }}) {
                console.log('Exam already ended, showing scoreboard immediately...');
                state.isEnded = true;

                // Hide split panels immediately
                elements.participantsPanel.style.display = 'none';
                elements.questionPanel.style.display = 'none';

                // Show scoreboard container
        ...
        function handleTestEnded(data) {
            ...
            // Show scoreboard container (ensure it's visible even if no scores yet)
            elements.scoreboardContainer.style.display = 'block';
            elements.scoreboardContainer.classList.add('active');
            elements.contentWrapper.classList.add('ended-mode');
            ...
            // Clear question state from localStorage
            clearQuestionState();
            // Save exam ended state to localStorage for persistence on refresh
            saveExamEndedState();
            ...
            if (data && (data.scoreboard || data.scores)) {
                ...
                // Fallback: Generate scoreboard from state.participants with their scores
                const scoreboardData = state.participants
                    .filter(p => p.score > 0) // Only include participants with scores
                    .map(p => ({
                        name: p.name,
                        university: p.university,
                        score: p.score || 0,
                        correct_answers: p.correct_answers || 0,
                        total_questions: p.total_questions || 10
                    }));
        ...
        async function fetchScoreboardFromServer() {
            try {
                ...
                if (response.data && response.data.scoreboard && response.data.scoreboard.length > 0) {
                    console.log('Scoreboard data received:', response.data.scoreboard.length, 'participants');
                    generateScoreboard(response.data.scoreboard);
                } else {
                    console.log('No scoreboard data available yet');
                    // Try again after a short delay
                    setTimeout(fetchScoreboardFromServer, 1000);
                }
            } catch (error) {
                console.error('Error fetching scoreboard:', error.message);
                // Try again after a short delay
                setTimeout(fetchScoreboardFromServer, 2000);
            }
        }
...
`;
        ...
        function highlightCorrectAnswer(correctOption) {
            ...
            options.forEach(option => {
                console.log('Option:', option.dataset.option, 'Correct:', correctOption, 'Match:', option.dataset
